...
const FALLBACK_AFTER_FRAMES = 420
...
async function ensureHitTestSource() {
  ...
  try {
    ...
    setLog('hit-test source를 만들 수 없습니다. plane-detection 또는 fallback을 사용합니다.')
  }
}
...
function onXRFrame(time, frame) {
  ...
  if (!placed && planeDetectionMode) {
    ...
    if (found) {
      ...
      setLog(`${product.name} | ${product.widthM}m × ${product.heightM}m | plane-detection 배치`)
    } else {
      frameWithoutPlane += 1
      const percent = Math.min(100, Math.round((frameWithoutPlane / FALLBACK_AFTER_FRAMES) * 100))
      setStatus(`실제 벽면 감지 중... ${percent}%`)
      setLog('벽에 질감이나 모서리가 보이도록 천천히 좌우로 움직이세요.')
    }

    if (!placed && frameWithoutPlane > FALLBACK_AFTER_FRAMES) {
      goFallback('vertical plane 감지 시간 초과')
      return
    }
  }
...
function placePosterInFrontOfCamera() {
  ...
  const center = cameraPosition.clone().add(cameraDirection.clone().multiplyScalar(DEFAULT_DISTANCE_M))
  ...
  setLog(`${product.name} | 추정거리 ${DEFAULT_DISTANCE_M.toFixed(1)}m | 수동 중앙 배치`)
}
...
function placeManual() {
  if (latestHitPose) {
    ...
    setLog('hit-test 결과를 사용했습니다. 벽과 안 맞으면 수동 모드로 전환하세요.')
    return
  }
...
function facePosterTowardCameraIfNeeded() {
  ...
  const posterNormal = new THREE.Vector3(0, 0, 1).applyQuaternion(posterRoot.quaternion).normalize()
